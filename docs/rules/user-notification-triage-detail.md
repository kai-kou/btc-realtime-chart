...
| `approval`（PR 作成前承認） | **非推奨**（CP-6 で PR 自律化済み）+ トリアージ組込済（#64） | 新規利用しない。PR 作成・マージはユーザー承認不要なので経路の既定は FYI（`baseline_mention=False`）で、本文が A-1〜A-6 に該当するときだけ `@mention` へ昇格する。`waiting` と違いチャンネルは切り替えず承認チャンネルに残す（スレッドの `OK` / `LGTM` 検出動線を壊さないため）|
| `publish` | FYI イベント分離 + トリアージ組込済（#64） | 完了報告系イベント（プロジェクト例: 配信完了 / スケジュール済み / 定例レポート）は `@mention` しない（`_PUBLISH_FYI_EVENTS`）。確認・節目アクション系イベント（プロジェクト例: 限定公開 / 公開前確認 / 本公開・A-2 相当）は `config/publish_events.yaml` の `fyi: false` を **経路宣言として尊重** して `@mention` を維持する（narrow な `_A2_PAT` では「限定公開」等を A-2 と判定できず、取り消し困難な公開の通知が黙って止まるため）。ただし本文が障害起因（公開失敗・エラー）かつ A 非該当なら降格して抑制し、FYI イベントでも A-1〜A-6（例: `detail` に課金枯渇）を検出したら昇格する |
...
> Hot 層予算の再棚卸し（#56）で `user-notification-triage.md` §6 から移設した（§1・§3・§5 の再掲であり、判定の実体を `tools/triage_notification.py` が持つため）。本ファイル内で `§6` は既に「専門チーム」節が使っているため、番号を付けずに見出しを立てている。

- [ ] `@mention` が A-1〜A-6 該当時のみ発火する（唯一の例外は `publish` の `fyi: false` イベント。§4 のとおり config の宣言を A-2 相当の経路宣言として扱う）
- [ ] 障害起因の通知が `@mention` されない（L-077 で自律修正）。`publish` の非 FYI イベントでも降格する
- [ ] A 区分通知に「具体的ユーザーアクション + 結果」が含まれる
- [ ] `triage_notification.py --self-test` と `slack_notify.py --self-test` が PASS
